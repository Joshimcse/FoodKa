...
`Foodka` is REST API server implementation built on top `Node.js` and `Express.js` with `Mongoose.js` for `MongoDB` integration. Access control follows `OAuth 2.0` spec with the help of `OAuth2orize` and `Passport.js`.
...
Created and maintained by Evgeny Aleksandrov ([@joshimcse](https://facebook.com/joshimcse)).
...
`Foodka` is available under the MIT license. See the [LICENSE.md](LICENSE.md) file for more info.